...
export function FavoriteClientsPage() {
	const [handleMount, handleUnmount] = useUnit([model.favoriteClientsPageMounted, model.favoriteClientsPageUnmounted])
									...
									>
										<TableFactory.THead bgColor='blue.500'>
											<TableFactory.Tr>
												...
												<TableFactory.Th color='white'>&nbsp;</TableFactory.Th>
											</TableFactory.Tr>
										</TableFactory.THead>
										<TableFactory.TBody columnLength={4} empty={{ empty: false }}>
											{item.meetingClients.map((client, index) => (
												<TableFactory.Tr key={index}>
													...
													<TableFactory.Td>
														<Button
															onClick={() => navigate(routes.meetingDetails({ meetingId: client.id.toString() }))}
														>
															<ExternalLinkIcon />
														</Button>
													</TableFactory.Td>
...
function SelectClientOfficeMeetingPlaceModal() {
	...
	const [clientToOfficeRoute, clientToOfficeRouteStatus] = useUnit([
		model.$$favoriteClientToOfficeRoute.$clientToOfficeRoute,
		model.$$favoriteClientToOfficeRoute.$clientToOfficeRouteStatus,
	])
		...
		>
			...
			<ModalContent>
				...
				<ModalBody position='relative' minH='600px'>
					<Stack minH='inherit'>
						<Stack direction='row'>
							<Badge colorScheme='blue'>{clientToOfficeRoute?.directionMatrix.destination_addresses}</Badge>
							<Badge colorScheme='blue'>{clientToOfficeRoute?.directionMatrix.origin_addresses}</Badge>
							<Badge colorScheme='blue'>
								{clientToOfficeRoute?.directionMatrix?.rows[0]?.elements[0]?.duration.value}
							</Badge>
							<Badge colorScheme='blue'>
								{clientToOfficeRoute?.directionMatrix?.rows[0]?.elements[0]?.distance.value}
							</Badge>
						</Stack>
						...
						>
							...
							<ClientToOfficeFactory.ClientToOfficeRoute model={model.$$favoriteClientToOfficeRoute} />
						</MapFactory.Map>
					</Stack>

					{clientToOfficeRouteStatus === 'pending' && (
						<Center position='absolute' top='50%' left='50%' zIndex='sticky'>
							<Spinner colorScheme='blue' />
						</Center>
					)}
